...
// ---------- El Camino de los Coyotes - 路线地图 ----------
function initCoyotesRouteMap() {
  const mapContainer = document.getElementById('coyotes-map');
  if (!mapContainer) return;

  // 初始化 Leaflet 地图
  const map = L.map('coyotes-map').setView([23.7400, -104.4200], 13);

  L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
    attribution: '&copy; contribuidores de OpenStreetMap'
  }).addTo(map);

  // 6 个叙事点坐标
  const puntos = [
    { lat: 23.7420, lng: -104.4180, titulo: 'Antigua Hacienda', audio: null },
    { lat: 23.7445, lng: -104.4155, titulo: 'El Ahuehuete', audio: null },
    { lat: 23.7460, lng: -104.4130, titulo: 'Puente de Piedra', audio: null },
    { lat: 23.7480, lng: -104.4105, titulo: 'Cueva de los Murciélagos', audio: null },
    { lat: 23.7500, lng: -104.4080, titulo: 'Mirador del Águila', audio: null },
    { lat: 23.7520, lng: -104.4055, titulo: 'Manantial Escondido', audio: null }
  ];
  ...
  puntos.forEach((p, i) => {
    L.marker([p.lat, p.lng])
      .addTo(map)
      .bindPopup(`(`<b>${p.titulo}</b><br>叙事点 ${i+1}`);
  });

  // 绘制路线（折线）
  const rutaCoords = puntos.map(p => [p.lat, p.lng]);
  L.polyline(rutaCoords, { color: '#B85C3A', weight: 4 }).addTo(map);

  // 模拟行走逻辑
  let currentPuntoIndex = 0;
  let distanciaRecorrida = 0;
  const totalKm = 12;
  const kmPorSegmento = totalKm / (puntos.length - 1);
  const factorCaminar = 0.12;
  const factorCoche = 0.33;

  const btnStart = document.getElementById('btn-start-walk');
  const btnReset = document.getElementById('btn-reset-walk');
  const statusDiv = document.getElementById('walk-status');
  const distEl = document.getElementById('dist-el');
  const carbonFill = document.getElementById('carbon-fill');
  const carbonText = document.getElementById('carbon-text');

  function actualizarUI() {
    if (distEl) distEl.textContent = distanciaRecorrida.toFixed(1) + ' km';
    const carbonoActual = distanciaRecorrida * factorCaminar;
    const carbonoSalvado = distanciaRecorrida * (factorCoche - factorCaminar);
    const pct = (distanciaRecorrida / totalKm) * 100;
    if (carbonFill) carbonFill.style.width = pct + '%';
    if (carbonText) {
      carbonText.textContent = `已排放 ${carbonoActual.toFixed(2)} kg CO₂ （较驾车节省 ${carbonoSalvado.toFixed(2)} kg）`;
    }
    if (statusDiv) {
      statusDiv.textContent = `已到达 ${currentPuntoIndex}/${puntos.length} 叙事点，剩余 ${(totalKm - distanciaRecorrida).toFixed(1)} km`;
    }
  }

  function avanzarAlPunto() {
    if (currentPuntoIndex >= puntos.length) {
      if (statusDiv) statusDiv.textContent = '🎉 你已走完整条 Coyotes 路线！零碳文化之旅完成。';
      if (btnStart) btnStart.disabled = true;
      return;
    }

    const punto = puntos[currentPuntoIndex];
    if (currentPuntoIndex > 0) {
      distanciaRecorrida += kmPorSegmento;
    } else {
    ...
    if (synth) {
      const utter = new SpeechSynthesisUtterance(`${punto.titulo}。这是一位长者留下的传说。`);
      utter.lang = 'es-MX';
      synth.cancel();
      synth.speak(utter);
    }
    if (statusDiv) statusDiv.textContent += ' 📢 ' + punto.titulo + ' 的故事已触发';
  ...
  if (btnReset) {
    btnReset.addEventListener('click', () => {
      ...
      if (btnStart) {
        btnStart.disabled = false;
        btnStart.textContent = '▶ 开始模拟行走';
      }
      btnReset.disabled = true;
      map.setView([23.7400, -104.4200], 13);
      actualizarUI();
  ...
  // 模拟录音按钮（叙事长椅）
  const btnRecord = document.getElementById('btn-record-bench');
  const recordStatus = document.getElementById('record-bench-status');
  
  if (btnRecord) {
    btnRecord.addEventListener('click', () => {
      if (recordStatus) {
        recordStatus.textContent = '（模拟）录音已上传。长者 María 的传说"Coyote 与月亮"已绑定至当前路段。';
      }
      // 模拟添加到地图上的一个新标记
      const fakeLat = 23.7450 + Math.random() * 0.01;
      const fakeLng = -104.4160 + Math.random() * 0.01;
      L.marker([fakeLat, fakeLng])
        .addTo(map)
        .bindPopup('🪶 María: Coyote 与月亮')
        .openPopup();
    });
  }
